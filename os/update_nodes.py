...
cd ~/.dev/python && git pull --no-rebase
cd ~/.dev/node && git pull --no-rebase
"""

fedora_command = """
dnf upgrade -y
cd /git/python && git pull --no-rebase
cd /git/node && git pull --no-rebase
"""

no_hostname_command = """
sudo apt-get update >> /dev/null
sudo apt-get install vim git git-lfs python3 python3-pip python3-venv default-jre curl tree -y >> /dev/null
"""
...
wsl_command = 'bash -c "apt-get update && apt-get upgrade -y && apt-get install vim git git-lfs python3 python3-pip python3-venv openjdk-11-jdk curl -y && cd ~/.dev/python && git pull --no-rebase && cd ~/.dev/node && git pull --no-rebase"'
